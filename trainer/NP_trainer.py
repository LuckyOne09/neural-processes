    ...
    def _train_epoch(self, epoch):
        ...
        for i, data in enumerate(self.data_loader):
            ...
            if i > 0:
                ...
                loss = self._loss(p_y_pred, y_target, q_target, q_context)
            # for t in range(self.task_num):     ### different zs
            #     z_sample, q_target, q_context = \
            #         self.neural_process['encoder'](x_context, y_context, x_target, y_target)
            #     p_y_pred = self.neural_process[t](x_context, y_context, x_target, z_sample)
            #
            #     if t > 0:
            #         loss = loss + self._loss(p_y_pred, y_target, q_target, q_context)
            #     else:
...
class SingleTaskNeuralProcessTrainer():
    """
    Class to handle training of Neural Processes for functions and images.
    ...
    def _train_epoch(self, epoch):
        ...
        for i, data in enumerate(self.data_loader):
            ...
            x, y = data
            x_context, y_context, x_target, y_target = \
                SingleTaskNeuralProcessTrainer.context_target_split(x, y, num_context, num_extra_target)

            # Use the same z or different zs for different tasks
            z_sample, q_target, q_context = \
                self.neural_process['encoder'](x_context, y_context, x_target, y_target)
            p_y_pred = self.neural_process[0](x_context, y_context, x_target, z_sample)
